...
export async function packageCodexPlugin(options = {}) {
  ...
  try {
    ...
    await rm(stagingRoot, { recursive: true, force: true, maxRetries: 10, retryDelay: 50 }).catch(() => {});
    if (backedUp) {
      await rm(outDir, { recursive: true, force: true, maxRetries: 10, retryDelay: 50 }).catch(() => {});
      await rename(backupRoot, outDir).catch(() => {});
    }
    return fail(skill.name, outDir, false, [`package write failed: ${error.message}`]);
  } finally {
    await rm(stagingRoot, { recursive: true, force: true, maxRetries: 10, retryDelay: 50 }).catch(() => {});
    await rm(backupRoot, { recursive: true, force: true, maxRetries: 10, retryDelay: 50 }).catch(() => {});
  }
