  ...
  for (const filePath of paths) {
    ...
    if (caps.write.scope === 'skill') {
      ...
      if (!confined.ok) {
        return deny(confined.reason === 'write path realpath failed'
          ? `write path realpath failed: ${filePath}`
          : `write escapes skill scope: ${filePath}`);
      }
    }
    if (caps.write.scope === 'project') {
      ...
      if (!confined.ok) {
        return deny(confined.reason === 'write path realpath failed'
          ? `write path realpath failed: ${filePath}`
          : `write escapes project scope: ${filePath}`);
      }
    }
  }
  return null;
...
export function parseApplyPatchPaths(patchText) {
  ...
  for (const line of lines) {
    const match = line.match(/^\*\*\*\s+(?:Update|Add|Delete)\s+File:\s+(.+?)\s*$/i)
      ?? line.match(/^\*\*\*\s+Move\s+to:\s+(.+?)\s*$/i);
