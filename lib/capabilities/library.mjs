...
function detectSourcePlugin(root, directory, homeOption) {
  ...
  const home = homeOption
    ? resolve(homeOption).replaceAll('\\', '/')
    : (process.env.USERPROFILE ? resolve(process.env.USERPROFILE).replaceAll('\\', '/') : null);
...
function displayPath(root, directory, home) {
  ...
  if (isInside(root, resolved)) return relative(root, resolved).replaceAll('\\', '/');
  if (home && isInside(home, resolved)) return `~/${relative(home, resolved).replaceAll('\\', '/')}`;
