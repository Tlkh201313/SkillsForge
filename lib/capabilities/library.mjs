...
import { loadAllSkills, normalizeExtraSkillRoots } from './skill-loader.mjs';
...
import { readSessionSummary, rememberSessionEvent } from './session.mjs';
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (url.pathname === '/api/project/select' || url.pathname === '/api/project/unselect') {
        ...
        const result = allowMutations
          ? await mutate(root, { ...body, home, config: options.config, sessionHost, extraSkillRoots })
          : {
            ok: true,
            dryRun: true,
            skill: body?.skill,
      ...
      if (url.pathname === '/api/session/remember') {
        const body = await readRequestJson(request);
        const result = allowMutations
          ? await rememberSessionEvent(root, body, { config: options.config, sessionHost })
          : { ok: true, dryRun: true, event: body, command: 'skillsforge session remember --query "<task>" --skill <id>' };
        sendJson(response, result);
        return;
      }
      const index = await buildLibraryIndex(root, { home, sessionHost, config: options.config, extraSkillRoots });
...
export async function listProjectSelection(root, options = {}) {
  const index = await buildLibraryIndex(root, options);
  return {
    ok: index.ok,
...
export async function selectProjectSkill(root, options = {}) {
...
export async function unselectProjectSkill(root, options = {}) {
  const skillId = options.skill ?? options.id;
  if (!skillId) throw new Error('--skill is required');
  const index = await buildLibraryIndex(root, options);
  const record = index.skills.find((skill) => skill.id === skillId || skill.key === skillId);
...
export function buildLibraryHtml(index, options = {}) {
  ...
  const problemsCount = (index.errors ?? []).length + (index.sourceDetails ?? [])
    .reduce((total, source) => total + (source.loadErrors?.length ?? 0), 0);
...
function buildSourceDetails(root, records, errors, context = {}) {
  ...
  for (const error of errors) {
    ...
    const sourceRoot = displayPath(root, dirname(errorDir || root), context.home);
...
function detectSourcePlugin(root, directory, homeOption) {
  return detectSourceInfo(root, directory, { home: homeOption }).id;
}
...
function detectSourceInfo(root, directory, options = {}) {
  ...
  const extraRoot = extraSkillRoots.find((item) => isInside(item, directory));
  if (extraRoot) {
    return {
      id: `extra:${basename(extraRoot) || 'root'}`,
      kind: 'extra-root',
      plugin: null,
      provider: 'extra',
      version: null
