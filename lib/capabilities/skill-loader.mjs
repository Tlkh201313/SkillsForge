...
export async function loadSkill(dir, options = {}) {
  ...
  const files = options.skipFileIntegrity === true
    ? [skillFile, ...(sidecarExists ? [sidecarFile] : [])]
...
async function skillsRootsFingerprint(skillsRoots) {
  ...
  for (const skillsRoot of skillsRoots) {
    try {
      ...
      // Per-skill mtimes so body/sidecar edits invalidate cache (not only dir count).
      for (const entry of dirs) {
        const skillDir = join(skillsRoot, entry.name);
        let stamp = entry.name;
        for (const file of ['SKILL.md', 'skillsforge.json']) {
          try {
            const fileStat = await stat(join(skillDir, file));
...
export function normalizeExtraSkillRoots(root, values = []) {
