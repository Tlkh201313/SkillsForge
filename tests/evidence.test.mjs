...
const fixedReports = {
  ...
  codexReport: {
    host: 'codex',
    plugin: { name: 'skillsforge', version: '0.4.0', skills: './skills/', interface: { displayName: 'SkillsForge', shortDescription: 'x' } },
...
test('live evidence run writes expected files and detects policy false-allow denominator', async (context) => {
  ...
  assert.equal(result.ok, true, JSON.stringify(result.reports?.policyAdversarial ?? result, null, 2));
  for (const name of [
    'manifest.json',
    'validation.json',
    'routing-eval.json',
