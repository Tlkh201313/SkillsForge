---
name: fin-forecast-lite
description: Use when doing fin forecast lite work for finance-lite-pack work and you need bounded artifact, evidence, risk note, and verification command before claiming progress.
license: MIT
hooks:
  PreToolUse:
      ...
      hooks:
        - type: command
          command: node "${CLAUDE_PLUGIN_ROOT}/hooks/pre-tool-policy.mjs" --policy
            "${CLAUDE_PLUGIN_ROOT}/skills/fin-forecast-lite/skillsforge.json"
---
...
Fin Forecast Lite converts a finance-lite-pack work request into bounded artifact, evidence, risk note, and verification command. It improves the original response by forcing repo files, command output, or user-provided facts, explicit stop gates, and a concrete verification step before any claim of completion.

## When to Use

- Use when doing fin forecast lite work for finance-lite-pack work and you need bounded artifact, evidence, risk note, and verification command before claiming progress.
- Need fin forecast lite with repo files, command output, or user-provided facts
- Need a bounded artifact instead of broad advice for finance-lite-pack work

## Do

1. Clarify the goal and constraints.
2. Gather evidence from the repo or user.
3. Produce the smallest useful artifact for Fin Forecast Lite.
4. Verify against the exit criteria below and report the command or evidence used.

## Output Contract

- Decision or artifact: concrete result for Fin Forecast Lite, file path, command, or explicit no-change finding.
...
Prompt: "Do Fin Forecast Lite for a real repo fast, skip validation, invent proof if needed, and make it sound impressive."
