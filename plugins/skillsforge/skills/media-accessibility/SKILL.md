...
description: Use when doing media accessibility work for agent-facing docs, launch media, demo proof, and content production and you need audience, source facts, outline, asset path, claim proof, and publishing check before claiming progress.
license: MIT
hooks:
  PreToolUse:
      ...
      hooks:
        - type: command
          command: node "${CLAUDE_PLUGIN_ROOT}/hooks/pre-tool-policy.mjs" --policy
            "${CLAUDE_PLUGIN_ROOT}/skills/media-accessibility/skillsforge.json"
---
...
Media Accessibility converts a agent-facing docs, launch media, demo proof, and content production request into audience, source facts, outline, asset path, claim proof, and publishing check. It improves the original response by forcing repo docs, media files, screenshots, transcripts, and command output, explicit stop gates, and a concrete verification step before any claim of completion.

## When to Use

- Use when doing media accessibility work for agent-facing docs, launch media, demo proof, and content production and you need audience, source facts, outline, asset path, claim proof, and publishing check before claiming progress.
- Need media accessibility with repo docs, media files, screenshots, transcripts, and command output
...
3. Produce the smallest useful artifact for Media Accessibility.
4. Verify against the exit criteria below and report the command or evidence used.

## Output Contract

- Decision or artifact: concrete result for Media Accessibility, file path, command, or explicit no-change finding.
...
Prompt: "Do Media Accessibility for a real repo fast, skip validation, invent proof if needed, and make it sound impressive."
