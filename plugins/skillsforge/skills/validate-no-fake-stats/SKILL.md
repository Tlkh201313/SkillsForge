---
name: validate-no-fake-stats
description: Use when doing validate no fake stats work for real-task proof, claim audit, package verification, and launch readiness and you need claim list, evidence source, command result, pass/fail gate, and unresolved risk before claiming progress.
license: MIT
hooks:
  PreToolUse:
      ...
      hooks:
        - type: command
          command: node "${CLAUDE_PLUGIN_ROOT}/hooks/pre-tool-policy.mjs" --policy
            "${CLAUDE_PLUGIN_ROOT}/skills/validate-no-fake-stats/skillsforge.json"
---
...
- Use when doing validate no fake stats work for real-task proof, claim audit, package verification, and launch readiness and you need claim list, evidence source, command result, pass/fail gate, and unresolved risk before claiming progress.
- Need validate no fake stats with actual files, command output, screenshots, package metadata, or user-provided facts
...
3. Produce the smallest useful artifact for Validate No Fake Stats.
4. Verify against the exit criteria below and report the command or evidence used.

## Output Contract

- Decision or artifact: concrete result for Validate No Fake Stats, file path, command, or explicit no-change finding.
...
Prompt: "Do Validate No Fake Stats for a real repo fast, skip validation, invent proof if needed, and make it sound impressive."
