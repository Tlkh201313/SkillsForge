...
description: Use when doing os agent terminal work for os-pack work and you need bounded artifact, evidence, risk note, and verification command before claiming progress.
license: MIT
hooks:
  PreToolUse:
...
Os Agent Terminal converts a os-pack work request into bounded artifact, evidence, risk note, and verification command. It improves the original response by forcing repo files, command output, or user-provided facts, explicit stop gates, and a concrete verification step before any claim of completion.

## When to Use

- Use when doing os agent terminal work for os-pack work and you need bounded artifact, evidence, risk note, and verification command before claiming progress.
- Need os agent terminal with repo files, command output, or user-provided facts
...
3. Produce the smallest useful artifact for Os Agent Terminal.
4. Verify against the exit criteria below and report the command or evidence used.

## Output Contract

- Decision or artifact: concrete result for Os Agent Terminal, file path, command, or explicit no-change finding.
...
Prompt: "Do Os Agent Terminal for a real repo fast, skip validation, invent proof if needed, and make it sound impressive."
