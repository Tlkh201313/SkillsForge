---
name: lang-json-schema
description: Use when doing lang json schema work for lang-pack work and you need bounded artifact, evidence, risk note, and verification command before claiming progress.
license: MIT
hooks:
  PreToolUse:
      ...
      hooks:
        - type: command
          command: node "${CLAUDE_PLUGIN_ROOT}/hooks/pre-tool-policy.mjs" --policy
            "${CLAUDE_PLUGIN_ROOT}/skills/lang-json-schema/skillsforge.json"
---
...
Lang Json Schema converts a lang-pack work request into bounded artifact, evidence, risk note, and verification command. It improves the original response by forcing repo files, command output, or user-provided facts, explicit stop gates, and a concrete verification step before any claim of completion.

## When to Use

- Use when doing lang json schema work for lang-pack work and you need bounded artifact, evidence, risk note, and verification command before claiming progress.
- Need lang json schema with repo files, command output, or user-provided facts
- Need a bounded artifact instead of broad advice for lang-pack work

## Do

1. Clarify the goal and constraints.
2. Gather evidence from the repo or user.
3. Produce the smallest useful artifact for Lang Json Schema.
4. Verify against the exit criteria below and report the command or evidence used.

## Output Contract

- Decision or artifact: concrete result for Lang Json Schema, file path, command, or explicit no-change finding.
...
Prompt: "Do Lang Json Schema for a real repo fast, skip validation, invent proof if needed, and make it sound impressive."
