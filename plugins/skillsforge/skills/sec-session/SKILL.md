---
name: sec-session
description: Use when doing sec session work for permission, privacy, boundary, dependency, and threat-model work and you need asset, actor, boundary, failure path, mitigation, and verification evidence before claiming progress.
license: MIT
hooks:
  PreToolUse:
      ...
      hooks:
        - type: command
          command: node "${CLAUDE_PLUGIN_ROOT}/hooks/pre-tool-policy.mjs" --policy
            "${CLAUDE_PLUGIN_ROOT}/skills/sec-session/skillsforge.json"
---
...
Sec Session converts a permission, privacy, boundary, dependency, and threat-model work request into asset, actor, boundary, failure path, mitigation, and verification evidence. It improves the original response by forcing code paths, configs, package metadata, policy files, and logs, explicit stop gates, and a concrete verification step before any claim of completion.

## When to Use

- Use when doing sec session work for permission, privacy, boundary, dependency, and threat-model work and you need asset, actor, boundary, failure path, mitigation, and verification evidence before claiming progress.
- Need sec session with code paths, configs, package metadata, policy files, and logs
...
3. Produce the smallest useful artifact for Sec Session.
4. Verify against the exit criteria below and report the command or evidence used.

## Output Contract

- Decision or artifact: concrete result for Sec Session, file path, command, or explicit no-change finding.
...
Prompt: "Do Sec Session for a real repo fast, skip validation, invent proof if needed, and make it sound impressive."
