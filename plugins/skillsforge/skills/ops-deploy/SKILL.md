---
name: ops-deploy
description: Use when doing ops deploy work for environment, CI, deployment, reliability, and operational readiness work and you need environment map, failing signal, command path, rollback option, and runbook note before claiming progress.
license: MIT
hooks:
  PreToolUse:
...
3. Produce the smallest useful artifact for Ops Deploy.
4. Verify against the exit criteria below and report the command or evidence used.

## Output Contract

- Decision or artifact: concrete result for Ops Deploy, file path, command, or explicit no-change finding.
...
Prompt: "Do Ops Deploy for a real repo fast, skip validation, invent proof if needed, and make it sound impressive."
