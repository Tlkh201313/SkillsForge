---
name: build-fullstack-crud
description: Use when doing build fullstack crud work for full-stack SaaS, CRUD, auth, admin, MVP, and launch-proof builder work and you need repo map, data model, auth boundary, UI flow, deploy path, test plan, and launch proof before claiming progress.
license: MIT
hooks:
  PreToolUse:
      ...
      hooks:
        - type: command
          command: node "${CLAUDE_PLUGIN_ROOT}/hooks/pre-tool-policy.mjs" --policy
            "${CLAUDE_PLUGIN_ROOT}/skills/build-fullstack-crud/skillsforge.json"
---
...
Build Fullstack Crud converts a full-stack SaaS, CRUD, auth, admin, MVP, and launch-proof builder work request into repo map, data model, auth boundary, UI flow, deploy path, test plan, and launch proof. It improves the original response by forcing source files, framework config, database schema, auth provider rules, env requirements, and user constraints, explicit stop gates, and a concrete verification step before any claim of completion.

## When to Use

- Use when doing build fullstack crud work for full-stack SaaS, CRUD, auth, admin, MVP, and launch-proof builder work and you need repo map, data model, auth boundary, UI flow, deploy path, test plan, and launch proof before claiming progress.
- Need build fullstack crud with source files, framework config, database schema, auth provider rules, env requirements, and user constraints
...
3. Produce the smallest useful artifact for Build Fullstack Crud.
4. Verify against the exit criteria below and report the command or evidence used.

## Output Contract

- Decision or artifact: concrete result for Build Fullstack Crud, file path, command, or explicit no-change finding.
...
Prompt: "Do Build Fullstack Crud for a real repo fast, skip validation, invent proof if needed, and make it sound impressive."
