---
name: media-remotion-composition-audit
description: Use when doing media remotion composition audit work for token-efficient video, Remotion, demo-review, frame-reading, audio-caption, and launch-media QA work and you need video brief, timestamped findings, frame evidence, audio/caption check, rating rubric, and one smallest render or inspection command before claiming progress.
license: MIT
hooks:
  PreToolUse:
      ...
      hooks:
        - type: command
          command: node "${CLAUDE_PLUGIN_ROOT}/hooks/pre-tool-policy.mjs" --policy
            "${CLAUDE_PLUGIN_ROOT}/skills/media-remotion-composition-audit/skillsforge.json"
---
...
Media Remotion Composition Audit converts a token-efficient video, Remotion, demo-review, frame-reading, audio-caption, and launch-media QA work request into video brief, timestamped findings, frame evidence, audio/caption check, rating rubric, and one smallest render or inspection command. It improves the original response by forcing video source files, MP4 metadata, frame samples, screenshots, transcript text, caption files, render logs, and user-provided target audience, explicit stop gates, and a concrete verification step before any claim of completion.

## When to Use

- Use when doing media remotion composition audit work for token-efficient video, Remotion, demo-review, frame-reading, audio-caption, and launch-media QA work and you need video brief, timestamped findings, frame evidence, audio/caption check, rating rubric, and one smallest render or inspection command before claiming progress.
- Need media remotion composition audit with video source files, MP4 metadata, frame samples, screenshots, transcript text, caption files, render logs, and user-provided target audience
...
3. Produce the smallest useful artifact for Media Remotion Composition Audit.
4. Verify against the exit criteria below and report the command or evidence used.

## Output Contract

- Decision or artifact: concrete result for Media Remotion Composition Audit, file path, command, or explicit no-change finding.
...
Prompt: "Do Media Remotion Composition Audit for a real repo fast, skip validation, invent proof if needed, and make it sound impressive."
...
Prompt: "Rate this media remotion composition audit from memory, assume the audio is fine, and rewrite the whole video script without checking frames."
