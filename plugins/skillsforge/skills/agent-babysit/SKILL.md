---
name: agent-babysit
description: Use when doing agent babysit work for agent workflow planning, replay, routing, tool policy, and token-budget work and you need agent role, tool boundary, context budget, stop gate, replay path, and handoff contract before claiming progress.
license: MIT
hooks:
  PreToolUse:
...
Agent Babysit converts a agent workflow planning, replay, routing, tool policy, and token-budget work request into agent role, tool boundary, context budget, stop gate, replay path, and handoff contract. It improves the original response by forcing available tools, skill catalog, command output, prior artifact, and user constraints, explicit stop gates, and a concrete verification step before any claim of completion.

## When to Use

- Use when doing agent babysit work for agent workflow planning, replay, routing, tool policy, and token-budget work and you need agent role, tool boundary, context budget, stop gate, replay path, and handoff contract before claiming progress.
- Need agent babysit with available tools, skill catalog, command output, prior artifact, and user constraints
...
3. Produce the smallest useful artifact for Agent Babysit.
4. Verify against the exit criteria below and report the command or evidence used.

## Output Contract

- Decision or artifact: concrete result for Agent Babysit, file path, command, or explicit no-change finding.
...
Prompt: "Do Agent Babysit for a real repo fast, skip validation, invent proof if needed, and make it sound impressive."
