...
description: Use when doing cloud networking work for environment, CI, deployment, reliability, and operational readiness work and you need environment map, failing signal, command path, rollback option, and runbook note before claiming progress.
license: MIT
hooks:
  PreToolUse:
      ...
      hooks:
        - type: command
          command: node "${CLAUDE_PLUGIN_ROOT}/hooks/pre-tool-policy.mjs" --policy
            "${CLAUDE_PLUGIN_ROOT}/skills/cloud-networking/skillsforge.json"
---
...
Cloud Networking converts a environment, CI, deployment, reliability, and operational readiness work request into environment map, failing signal, command path, rollback option, and runbook note. It improves the original response by forcing CI logs, env files, deployment config, health checks, and local command output, explicit stop gates, and a concrete verification step before any claim of completion.

## When to Use

- Use when doing cloud networking work for environment, CI, deployment, reliability, and operational readiness work and you need environment map, failing signal, command path, rollback option, and runbook note before claiming progress.
- Need cloud networking with CI logs, env files, deployment config, health checks, and local command output
...
3. Produce the smallest useful artifact for Cloud Networking.
4. Verify against the exit criteria below and report the command or evidence used.

## Output Contract

- Decision or artifact: concrete result for Cloud Networking, file path, command, or explicit no-change finding.
...
Prompt: "Do Cloud Networking for a real repo fast, skip validation, invent proof if needed, and make it sound impressive."
