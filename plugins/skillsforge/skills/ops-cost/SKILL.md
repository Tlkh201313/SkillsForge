---
name: ops-cost
description: Use when doing ops cost work for environment, CI, deployment, reliability, and operational readiness work and you need environment map, failing signal, command path, rollback option, and runbook note before claiming progress.
license: MIT
hooks:
  PreToolUse:
      ...
      hooks:
        - type: command
          command: node "${CLAUDE_PLUGIN_ROOT}/hooks/pre-tool-policy.mjs" --policy
            "${CLAUDE_PLUGIN_ROOT}/skills/ops-cost/skillsforge.json"
---
...
- Use when doing ops cost work for environment, CI, deployment, reliability, and operational readiness work and you need environment map, failing signal, command path, rollback option, and runbook note before claiming progress.
- Need ops cost with CI logs, env files, deployment config, health checks, and local command output
...
3. Produce the smallest useful artifact for Ops Cost.
4. Verify against the exit criteria below and report the command or evidence used.

## Output Contract

- Decision or artifact: concrete result for Ops Cost, file path, command, or explicit no-change finding.
...
Prompt: "Do Ops Cost for a real repo fast, skip validation, invent proof if needed, and make it sound impressive."
