---
name: data-backfill
description: Use when doing data backfill work for analytics, events, schema, metrics, dashboard, and data-quality work and you need event/schema definition, owner, validation rule, sample query, and failure path before claiming progress.
license: MIT
hooks:
  PreToolUse:
...
Data Backfill converts a analytics, events, schema, metrics, dashboard, and data-quality work request into event/schema definition, owner, validation rule, sample query, and failure path. It improves the original response by forcing schema files, SQL, analytics config, pipeline logs, and dashboard specs, explicit stop gates, and a concrete verification step before any claim of completion.

## When to Use

- Use when doing data backfill work for analytics, events, schema, metrics, dashboard, and data-quality work and you need event/schema definition, owner, validation rule, sample query, and failure path before claiming progress.
- Need data backfill with schema files, SQL, analytics config, pipeline logs, and dashboard specs
...
3. Produce the smallest useful artifact for Data Backfill.
4. Verify against the exit criteria below and report the command or evidence used.

## Output Contract

- Decision or artifact: concrete result for Data Backfill, file path, command, or explicit no-change finding.
...
Prompt: "Do Data Backfill for a real repo fast, skip validation, invent proof if needed, and make it sound impressive."
