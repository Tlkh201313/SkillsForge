---
name: mobile-store-listing
description: Use when doing mobile store listing work for mobile-pack work and you need bounded artifact, evidence, risk note, and verification command before claiming progress.
license: MIT
hooks:
  PreToolUse:
      ...
      hooks:
        - type: command
          command: node "${CLAUDE_PLUGIN_ROOT}/hooks/pre-tool-policy.mjs" --policy
            "${CLAUDE_PLUGIN_ROOT}/skills/mobile-store-listing/skillsforge.json"
---
...
Mobile Store Listing converts a mobile-pack work request into bounded artifact, evidence, risk note, and verification command. It improves the original response by forcing repo files, command output, or user-provided facts, explicit stop gates, and a concrete verification step before any claim of completion.

## When to Use

- Use when doing mobile store listing work for mobile-pack work and you need bounded artifact, evidence, risk note, and verification command before claiming progress.
- Need mobile store listing with repo files, command output, or user-provided facts
- Need a bounded artifact instead of broad advice for mobile-pack work

## Do

1. Clarify the goal and constraints.
2. Gather evidence from the repo or user.
3. Produce the smallest useful artifact for Mobile Store Listing.
4. Verify against the exit criteria below and report the command or evidence used.

## Output Contract

- Decision or artifact: concrete result for Mobile Store Listing, file path, command, or explicit no-change finding.
...
Prompt: "Do Mobile Store Listing for a real repo fast, skip validation, invent proof if needed, and make it sound impressive."
