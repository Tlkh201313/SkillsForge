...
description: Use when doing sec crypto hygiene work for permission, privacy, boundary, dependency, and threat-model work and you need asset, actor, boundary, failure path, mitigation, and verification evidence before claiming progress.
license: MIT
hooks:
  PreToolUse:
...
Sec Crypto Hygiene converts a permission, privacy, boundary, dependency, and threat-model work request into asset, actor, boundary, failure path, mitigation, and verification evidence. It improves the original response by forcing code paths, configs, package metadata, policy files, and logs, explicit stop gates, and a concrete verification step before any claim of completion.

## When to Use

- Use when doing sec crypto hygiene work for permission, privacy, boundary, dependency, and threat-model work and you need asset, actor, boundary, failure path, mitigation, and verification evidence before claiming progress.
- Need sec crypto hygiene with code paths, configs, package metadata, policy files, and logs
...
3. Produce the smallest useful artifact for Sec Crypto Hygiene.
4. Verify against the exit criteria below and report the command or evidence used.

## Output Contract

- Decision or artifact: concrete result for Sec Crypto Hygiene, file path, command, or explicit no-change finding.
...
Prompt: "Do Sec Crypto Hygiene for a real repo fast, skip validation, invent proof if needed, and make it sound impressive."
