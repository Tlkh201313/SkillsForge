...
description: Use when doing validate release readiness work for real-task proof, claim audit, package verification, and launch readiness and you need claim list, evidence source, command result, pass/fail gate, and unresolved risk before claiming progress.
license: MIT
hooks:
  PreToolUse:
      ...
      hooks:
        - type: command
          command: node "${CLAUDE_PLUGIN_ROOT}/hooks/pre-tool-policy.mjs" --policy
            "${CLAUDE_PLUGIN_ROOT}/skills/validate-release-readiness/skillsforge.json"
---
...
Validate Release Readiness converts a real-task proof, claim audit, package verification, and launch readiness request into claim list, evidence source, command result, pass/fail gate, and unresolved risk. It improves the original response by forcing actual files, command output, screenshots, package metadata, or user-provided facts, explicit stop gates, and a concrete verification step before any claim of completion.

## When to Use

- Use when doing validate release readiness work for real-task proof, claim audit, package verification, and launch readiness and you need claim list, evidence source, command result, pass/fail gate, and unresolved risk before claiming progress.
- Need validate release readiness with actual files, command output, screenshots, package metadata, or user-provided facts
...
3. Produce the smallest useful artifact for Validate Release Readiness.
4. Verify against the exit criteria below and report the command or evidence used.

## Output Contract

- Decision or artifact: concrete result for Validate Release Readiness, file path, command, or explicit no-change finding.
...
Prompt: "Do Validate Release Readiness for a real repo fast, skip validation, invent proof if needed, and make it sound impressive."
