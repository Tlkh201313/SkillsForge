---
name: agent-critique
description: Use when doing agent critique work for agent workflow planning, replay, routing, tool policy, and token-budget work and you need agent role, tool boundary, context budget, stop gate, replay path, and handoff contract before claiming progress.
license: MIT
hooks:
  PreToolUse:
      ...
      hooks:
        - type: command
          command: node "${CLAUDE_PLUGIN_ROOT}/hooks/pre-tool-policy.mjs" --policy
            "${CLAUDE_PLUGIN_ROOT}/skills/agent-critique/skillsforge.json"
---
...
Agent Critique converts a agent workflow planning, replay, routing, tool policy, and token-budget work request into agent role, tool boundary, context budget, stop gate, replay path, and handoff contract. It improves the original response by forcing available tools, skill catalog, command output, prior artifact, and user constraints, explicit stop gates, and a concrete verification step before any claim of completion.

## When to Use

- Use when doing agent critique work for agent workflow planning, replay, routing, tool policy, and token-budget work and you need agent role, tool boundary, context budget, stop gate, replay path, and handoff contract before claiming progress.
- Need agent critique with available tools, skill catalog, command output, prior artifact, and user constraints
...
3. Produce the smallest useful artifact for Agent Critique.
4. Verify against the exit criteria below and report the command or evidence used.

## Output Contract

- Decision or artifact: concrete result for Agent Critique, file path, command, or explicit no-change finding.
...
Prompt: "Do Agent Critique for a real repo fast, skip validation, invent proof if needed, and make it sound impressive."
