...
Current inventory: **367** skills · **26** packs · **10** profiles · **100** workflows · **98** agents · **124** commands.
...
| Scale | 367 skills / 26 packs / 98 agents / 124 commands / 100 workflows |
| Operator path | `vibe` · `lib recommend` · `workflows recommend` · `auto run --read-only` · `wb status` |
...
| Catalog | 367/367 skills ship `skillsforge.json` |
...
node plugins/skillsforge/bin/skillsforge.mjs lib recommend --query "safe refactor" --session-host codex
node plugins/skillsforge/bin/skillsforge.mjs workflows recommend --query "ship a release"
...
> **Not on npm yet.** Do not use `npx skillsforge` — invoke the repo binary as shown above.
...
| Productivity catalog | **367** skills, **26** packs, **10** profiles |
| Workflows | **100** dry-run definitions under `plugins/skillsforge/workflows/` |
| Agents / commands | **98** agents · **124** command shims |
| Library | `lib build|update|serve|recommend` — local HTML + AI index |
...
| Depth honesty | Structured heroes + lean domain scaffolds until expanded |
| Universal hosts | `hosts`, `install --hosts all|detected`, `--custom-host` |
| Thin MCP | Trust tools + read-only recommend |
...
Expected PowerShell helpers: `sf-status`, `sf-tree`, `sf-find`, `sf-grep`, `sf-diff`, `sf-errors`, `sf-bigfiles`, `sf-recent`, `sf-ports`, `sf-proof`, `sf-lib`, `sf-lib-update`, `sf-recommend`, `sf-workflow`, `sf-auto`.
...
No Ruflo swarms, AgentDB, consensus, OS sandboxing, or third-party attestation. Library UI is local, not a cloud dashboard. Domain packs are **lean scaffolds**, not dumps from skills.sh.
...
- **Honest depth:** structured heroes + lean domains; eight auto-route heroes.
